	...
	<div class="container bg-white">
		...
		<div class="row">
            ...
            <ul class="nav rounded shadow">
            	...
            	<li class="nav-item btn btn-success m-3" ><a href="PrincipalfacultyLeaves.jsp" class="nav-link text-light active ">Back</a></li>
            </ul>
		  	</div>
     		<div class="row">
     	...
     	<%
     	   //jdbc
     		...
     		ResultSet rs = st.executeQuery("select * from onduty");
     		while(rs.next()){
     			%>
     		     <tr>
     		        <td><%=rs.getInt(1) %></td>
					...
					<td><a href="PfacultyLeaveApprove.jsp?fid=<%=rs.getString(2)%>&status=<%="Approve" %> &LID=<%=rs.getInt(1) %>" class="btn btn-success" >Approve</a>
     		        	<a href="PfacultyLeaveReject.jsp?fid=<%=rs.getString(2)%>&status=<%="Reject" %>" class="btn btn-danger" >Reject</a>
     		     </tr>
     		<%}
