	...
	<div class="container bg-white">
		...
		<div class="row">
     	...
     	<%
     		...
     		ResultSet rs = st.executeQuery("select * from cl where Designation='HOD' and status = 'Pending'");
     		while(rs.next()){
     			%>
     		     <tr>
     		        <td><%=rs.getInt(1) %></td>
     		     ...
     		     <td><a href="PrincipalLeaveApprove.jsp?fid=<%=rs.getString(2)%>&status=<%="Approve" %> &LID=<%=rs.getInt(1) %>" class="btn btn-success" >Approve</a>
     		        	<a href="PrincipalLeaveReject.jsp?fid=<%=rs.getString(2)%>&status=<%="Reject" %>" class="btn btn-danger" >Reject</a>
     		        </td>
     		     </tr>
     		<%}
